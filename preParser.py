...
class PreParser(object):
    ...
    def push_builtins_to_table(self, table):
        ...
        def create_exit_func():
            return FunctionSymbol('int', 'exit', AST.ParametersList([AST.Parameter('int', 'int_to_print')]))     #???????
...
# # Parse the teslang source code to build the AST
# parser = TeslangParser()
# ast = parser.parse(source_code)

# # Create a symbol table
# pre_parser = PreParser()
# symbol_table = pre_parser.visit_Program(ast, None)